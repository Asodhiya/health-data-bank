...
async def create_form(form_data: SurveyCreate, db: AsyncSession = Depends(get_rls_db),current_user: User = Depends(check_current_user)):
...
async def publish_preview(form_id: UUID, group_ids: str, db: AsyncSession = Depends(get_rls_db)):
...
async def list_groups_for_publish(db: AsyncSession = Depends(get_rls_db)):
