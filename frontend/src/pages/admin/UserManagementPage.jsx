...
export default function UserManagementPage() {
  ...
  const handleDelete = async (u, mode) => {
    ...
    if (mode === "anonymize") { const a = { ...u, firstName: "Deleted", lastName: `User #${u.id}`, email: `deleted_${u.id}@removed.local`, phone: "—", status: "inactive" }; setUsers(p => p.map(x => x.id === u.id ? a : x)); msg(`User anonymized.`); } else { setUsers(p => p.filter(x => x.id !== u.id)); msg(`User permanently deleted.`); }
