...
/* ── Chip select (matches existing app pattern) ── */
...
/* ── Field wrapper with required-field awareness ── */
...
export default function CaretakerOnboardingPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (missingFields.length > 0) {
      ...
      const firstMissing = document.getElementById(`field-${keys[0]}`);
      if (firstMissing) {
    ...
    try {
      await api.caretakerUpdateProfile(payload);
      // Re-fetch auth context so onboarding_completed updates and the route gate opens
      await refetch();
      setSuccess(true);
    } catch (err) {
        ...
        <div className="w-full bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-slate-200/60 p-6 sm:p-8">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-slate-500">
              Help your participants and colleagues know who you are.
            </p>
          </div>
          ...
          <div className="border border-slate-100 rounded-xl p-5 mb-5">
            ...
            <Field id="field-credentials" label="Credentials" hint="Displayed next to your name on participant-facing pages">
              <input
                className={inputCls}
                placeholder="e.g. PhD, RN, CSEP-CEP"
            ...
            <Field id="field-bio" label="Short Bio">
              <textarea
                ...
                rows={3}
                placeholder="Tell participants and colleagues about your background..."
                value={form.bio}
          ...
          <div className="border border-slate-100 rounded-xl p-5 mb-6">
            ...
            <Field id="field-availableDays" label="Available Days" required missing={isMissing("availableDays")} shake={isShaking("availableDays")}>
              <ChipSelect
